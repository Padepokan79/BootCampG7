...
public class AnalisSistem extends Gaji implements BasicSalary, Allowance, SalaryCuts{
	...
	public void setBasicSalary(int masaKerja) {
		if (getPengalaman().equalsIgnoreCase("Junior") ) {
			if (masaKerja>=arraySaJunior.size()) {
				masaKerja = arraySaJunior.size()-1;
			}
			basicSalary = umk*arraySaJunior.get(masaKerja);
		}else if (getPengalaman().equalsIgnoreCase("Middle")) {
			if (masaKerja>=arraySaMiddle.size()) {
				masaKerja = arraySaMiddle.size()-1;
			}
			basicSalary = umk*arraySaMiddle.get(masaKerja);
		}else if (getPengalaman().equalsIgnoreCase("Senior")) {
			if (masaKerja>=arraySaSenior.size()) {
				masaKerja = arraySaSenior.size()-1;
			}
			basicSalary = umk*arraySaSenior.get(masaKerja);
		}
	}
