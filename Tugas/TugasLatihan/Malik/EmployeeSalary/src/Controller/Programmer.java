...
public class Programmer extends Employee implements BasicSalary, Allowance, SalaryCuts{
	...
	public void setBasicSalary(int masaKerja) {
		if (getPengalaman().equalsIgnoreCase("Junior") ) {
			if (masaKerja>=arrayPgJunior.length) {
				masaKerja = arrayPgJunior.length-1;
			}
			basicSalary = umk*arrayPgJunior[masaKerja];
		}else if (getPengalaman().equalsIgnoreCase("Middle")) {
			if (masaKerja>=arrayPgMiddle.length) {
				masaKerja = arrayPgMiddle.length-1;
			}
			basicSalary = umk*arrayPgMiddle[masaKerja];
		}else if (getPengalaman().equalsIgnoreCase("Senior")) {
			if (masaKerja>=arrayPgSenior.length) {
				masaKerja = arrayPgSenior.length-1;
			}
			basicSalary = umk*arrayPgSenior[masaKerja];
		}
	}
