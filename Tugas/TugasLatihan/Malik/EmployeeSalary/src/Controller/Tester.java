...
public class Tester extends Employee implements BasicSalary, Allowance, SalaryCuts{
	...
	public void setBasicSalary(int masaKerja) {
		if (getPengalaman().equalsIgnoreCase("Junior") ) {
			if (masaKerja>=arrayTsJunior.length) {
				masaKerja = arrayTsJunior.length-1;
			}
			basicSalary = umk*arrayTsJunior[masaKerja];
		}else if (getPengalaman().equalsIgnoreCase("Middle")) {
			if (masaKerja>=arrayTsMiddle.length) {
				masaKerja = arrayTsMiddle.length-1;
			}
			basicSalary = umk*arrayTsMiddle[masaKerja];
		}else if (getPengalaman().equalsIgnoreCase("Senior")) {
			if (masaKerja>=arrayTsSenior.length) {
				masaKerja = arrayTsSenior.length-1;
			}
			basicSalary = umk*arrayTsSenior[masaKerja];
		}
	}
