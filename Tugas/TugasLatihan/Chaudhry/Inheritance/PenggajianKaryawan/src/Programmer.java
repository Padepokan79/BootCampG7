...
public class Programmer extends Karyawan implements PenentuGapok, NominalDefault {
	...
	public Programmer(String nama, String nik) {
		super(nama, nik);
	}
	
	void setFixBug(int fix) {
		fixBug = fix;
	}
	int getFixBug() {
		return fixBug;
	...
	void hitungGapok(int lamaKerja, String tingkatan) {
		if(tingkatan.equals("Junior")) {
			if(lamaKerja >= 0 && lamaKerja <= 1) {
				gapok = getUmk(getPenempatanKerja())*pgJunior[0];
			
			}else if(lamaKerja > 1 && lamaKerja <= 3) {
				gapok = getUmk(getPenempatanKerja())*pgJunior[1];
			
			}else if(lamaKerja > 3) {
				gapok = getUmk(getPenempatanKerja())*pgJunior[2];
			}
		
		}else if(tingkatan.equals("Middle")) {
			if(lamaKerja >= 0 && lamaKerja <= 2) {
				gapok = getUmk(getPenempatanKerja())*pgMiddle[0];
			
			}else if(lamaKerja > 2 && lamaKerja <= 4) {
				gapok = getUmk(getPenempatanKerja())*pgMiddle[1];
			
			}else if(lamaKerja > 4) {
				gapok = getUmk(getPenempatanKerja())*pgMiddle[2];
			}
			
		}else if(tingkatan.equals("Senior")) {
			if(lamaKerja >= 0 && lamaKerja <= 2) {
				gapok = getUmk(getPenempatanKerja())*pgSenior[0];
			
			}else if(lamaKerja > 2 && lamaKerja <= 4) {
				gapok = getUmk(getPenempatanKerja())*pgSenior[1];
			
			}else if(lamaKerja > 4) {
				gapok = getUmk(getPenempatanKerja())*pgSenior[2];
			}
		}
	}
	double getGapok() {
	...
	public void tampilData() {
		...
		System.out.println("Fix Bug             : "+getFixBug());
