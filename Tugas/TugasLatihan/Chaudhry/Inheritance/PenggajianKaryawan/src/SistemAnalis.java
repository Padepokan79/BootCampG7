...
public class SistemAnalis extends Karyawan implements PenentuGapok, NominalDefault {
	private int lembur, fixBug, lamaKerja, jmlAnak;
	...
	public SistemAnalis(String nama, String nik) {
		super(nama, nik);
	}
	...
	void hitungGapok(int lamaKerja, String tingkatan) {
		if(tingkatan.equals("Junior")) {
			if(lamaKerja >= 0 && lamaKerja <= 1) {
				gapok = getUmk(getPenempatanKerja())*saJunior[0];
			
			}else if(lamaKerja > 1 && lamaKerja <= 3) {
				gapok = getUmk(getPenempatanKerja())*saJunior[1];
			
			}else if(lamaKerja > 3) {
				gapok = getUmk(getPenempatanKerja())*saJunior[2];
			}
		
		}else if(tingkatan.equals("Middle")) {
			if(lamaKerja >= 0 && lamaKerja <= 2) {
				gapok = getUmk(getPenempatanKerja())*saMiddle[0];
			
			}else if(lamaKerja > 2 && lamaKerja <= 4) {
				gapok = getUmk(getPenempatanKerja())*saMiddle[1];
			
			}else if(lamaKerja > 4) {
				gapok = getUmk(getPenempatanKerja())*saMiddle[2];
			}
			
		}else if(tingkatan.equals("Senior")) {
			if(lamaKerja >= 0 && lamaKerja <= 2) {
				gapok = getUmk(getPenempatanKerja())*saSenior[0];
			
			}else if(lamaKerja > 2 && lamaKerja <= 4) {
				gapok = getUmk(getPenempatanKerja())*saSenior[1];
			
			}else if(lamaKerja > 4) {
				gapok = getUmk(getPenempatanKerja())*saSenior[2];
			}
		}
	}
	double getGapok() {
	...
	void hitungLembur(double lamaLembur) {
		uangLembur = 1.5 * 1.0/173.0 * (getGapok() + getTunjanganPegawai() + getTunKeluarga() + getTunTransport());
