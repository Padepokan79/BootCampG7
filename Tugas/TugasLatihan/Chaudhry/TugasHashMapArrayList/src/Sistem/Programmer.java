...
public class Programmer extends Karyawan implements UmkKota, PengalamanKerja, Tunjangan, Potongan {
	...
	int lembur, fixBug, masaKerja;
	boolean statusKawin;
	double[] arrPkp = new double[4];
	
	public Programmer(String nama, String nik, String penempatan, String posisi, String tingkatan, int masaKerja, int fixBug, int lembur, boolean statusKawin) {
	...
	public double getGapok(int masaKerja) {
		if(getTingkatan().equals("Junior")) {
			if(masaKerja >= arrPgJunior.size()) {
				masaKerja = arrPgJunior.size() - 1;
			}
			gapok = getUmk()*arrPgJunior.get(masaKerja);
			
		}else if(getTingkatan().equals("Middle")) {
			if(masaKerja >= arrPgMiddle.size()) {
				masaKerja = arrPgMiddle.size() - 1;
			}
			gapok = getUmk()*arrPgMiddle.get(masaKerja);
			
		}else if(tingkatan.equals("Senior")) {
			if(masaKerja >= arrPgSenior.size()) {
				masaKerja = arrPgSenior.size() - 1;
			}
			gapok = getUmk()*arrPgSenior.get(masaKerja);
		}
		return Math.round(gapok);
	}
	
	public double getBonus() {
		bonus = fixBug*bugFixing;
		if(bonus > 500000) {
			bonus = 500000;
		}
		return bonus;
	...
	public void tampilData() {
		...
		System.out.println("Fix Bug        : "+fixBug);
