...
public class SistemAnalis extends Karyawan implements UmkKota, PengalamanKerja, Tunjangan, Potongan {

	private double umk, gapok, gajiBersih, tunPegawai, tunTransport, tunKeluarga = 0, uangLembur, potBpjs, gajiKotor, nettoBulan, nettoSetahun, biayaJabat, iuranPen, ptkp, pkp, pphSetahun, pphSebulan;
	int lembur, masaKerja;
	boolean statusKawin;
	double[] arrPkp = new double[4];
	
	public SistemAnalis(String nama, String nik, String penempatan, String posisi, String tingkatan, int masaKerja, int lembur, boolean statusKawin) {
	...
	public double getGapok(int masaKerja) {
		if(getTingkatan().equals("Junior")) {
			if(masaKerja >= arrSaJunior.size()) {
				masaKerja = arrSaJunior.size() - 1;
			}
			gapok = getUmk()*arrSaJunior.get(masaKerja);
			
		}else if(getTingkatan().equals("Middle")) {
			if(masaKerja >= arrSaMiddle.size()) {
				masaKerja = arrSaMiddle.size() - 1;
			}
			gapok = getUmk()*arrSaMiddle.get(masaKerja);
			
		}else if(tingkatan.equals("Senior")) {
			if(masaKerja >= arrSaSenior.size()) {
				masaKerja = arrSaSenior.size() - 1;
			}
			gapok = getUmk()*arrSaSenior.get(masaKerja);
		}
	...
	public double getUangLembur() {
		uangLembur = ( 1.5 * 1.0/173.0 * (getGapok(getMasaKerja()) + getTunPegawai() + getTunKeluarga(getStatusKawin()) + getTunTransport()) )*lembur;
	...
	public double getGajiBersih() {
		gajiBersih = ( getGapok(getMasaKerja()) + getTunPegawai() + getTunTransport() + getTunKeluarga(getStatusKawin()) + getUangLembur() ) - ( getPotBpjs()*2 + getPotPph(getStatusKawin()) );
		return gajiBersih;
	}
