...
public class Tester extends Karyawan implements BasicSalary, Tunjangan, Potongan {
	...
	public Tester(String nama, String nik, String penempatan, String pengalaman, int masaKerja, boolean married, int testCase, int jamLembur) {
		super(nama, nik, penempatan, "Tester", pengalaman);
		setGapok(masaKerja);
		
		hitungTunjangan(married);
		hitungBonus(testCase);
	...
	public Tester(String nama, String nik, String penempatan, String pengalaman, int masaKerja, int testCase, int jamLembur) {
		super(nama, nik, penempatan, "Tester", pengalaman);
		setGapok(masaKerja);
	...
	private void hitungBonus(int testCase) {
		bonus=(testCase/100)*25000;
		if(bonus>500000)
			bonus=500000;
	}
	...
	public void setGapok(int masaKerja) {
		setUMK();
		if(getPengalaman().equalsIgnoreCase("Junior")) {
			if(masaKerja>=tsJunior.size())
				masaKerja=tsJunior.size()-1;
			gapok=umk*tsJunior.get(masaKerja);
		}
		else if(getPengalaman().equalsIgnoreCase("Middle")) {
			if(masaKerja>=tsMiddle.size())
				masaKerja=tsMiddle.size()-1;
			gapok=umk*tsMiddle.get(masaKerja);
		}
		else if(getPengalaman().equalsIgnoreCase("Senior")) {
			if(masaKerja>=tsSenior.size())
				masaKerja=tsSenior.size()-1;
			gapok=umk*tsSenior.get(masaKerja);
		}
	}
